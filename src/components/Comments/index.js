  ...
  render() {
    ...
    return (
      <div className='flex flex-col'>
        <div className='flex flex-row justify-center items-center px-6 pt-10'>
          <form onSubmit={this.addComment} className='flex flex-col justify-start items-start'>
            ...
            <textarea onChange={this.commentText} placeholder='Explain here' className='pl-2 border-2 border-gray-500 mt-2 rounded' id="txtid" name="txtname" rows="4" cols="25" maxlength="200">
            </textarea>
            <button className='bg-teal-500 hover:bg-teal-600 py-2 px-4 rounded text-white shadow mt-2'>Add Comment</button>
          </form>

          <img className='w-60' src='https://assets.ccbp.in/frontend/react-js/comments-app/comments-img.png' />
        </div>

        <div className='flex flex-row ml-8 mt-4 justify-start items-center'>
          <button className='w-8 h-8 bg-teal-500 mr-2 rounded-sm'>{currentList.length}</button>
          <p>Comments</p>
        </div>

        <ul>
          {currentList.map(eachitem => (
            <li className='mx-8 my-4 border-2 rounded border-gray-500 p-4 flex flex-row justify-between'>
              <div>
                <div className='mb-2 flex flex-row justify-start items-center'>
                  <button className='mr-2 h-10 w-10 rounded-full bg-orange-500'>S</button>
